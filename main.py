...
### testdata ###
testdata = []
fn, wav_data = wavfile.read('../../DATA/TIMITGAN/1_0.wav')
wave = (2. / 65535.) * (wav_data.astype(np.float32) - 32767) + 1.
...
steganalysis2 = torch.nn.DataParallel(model.Steganalysis().to(device), device_ids=use_devices)
steganalysis2.cuda()
# print(steganalysis)
...
l1_smooth_loss = torch.nn.SmoothL1Loss().to(device)
...
discriminator.load_state_dict(torch.load('./Snetpoint/Dnet32.pkl'))
generator.load_state_dict(torch.load('./Snetpoint/Gnet32.pkl'))
steganalysis.load_state_dict(torch.load('./Snetpoint/chen20.pkl'))
...
for epoch in range(train_epoch):
    ...
    for i, (ori, _) in enumerate(random_data_loader):
        ...
        for depoch in range(D_num):
            ...
            stego_audio_d = (2. / 65535.) * (stego_audio.astype(np.float32) - 32767) + 1.
            stego_audio_d = stego_audio_d.reshape(-1, 1, 16384)
            stego_audio_d = torch.from_numpy(stego_audio_d)
            output = discriminator(stego_audio_d.detach())
        ...
        for d_gepoch in range(G_num):
            ...
            g_loss_l1 = torch.mean(l1_dist)

            stego_audio = embed(generated_outputs)

            stego_audio1 = (2. / 65535.) * (stego_audio.astype(np.float32) - 32767) + 1.
            stego_audio1 = stego_audio1.reshape(-1, 1, 16384)
            stego_audio1 = torch.from_numpy(stego_audio1)
            stego_audio1 = stego_audio1.to(device)
            output1 = discriminator(stego_audio1)
            g_loss_d = 0.5 * torch.mean((output1 - 1.0) ** 2)

            if epoch > S_epoch:
                stego_audio2 = stego_audio.astype(np.float32)
                stego_audio2 = stego_audio2.reshape(-1, 1, 16384)
                stego_audio2 = torch.from_numpy(stego_audio2).to(device)
                output2 = steganalysis(stego_audio2)
                bceloss = bce_loss(output2, ones)
                if bceloss < best:
                    best = bceloss
                    gpath2 = './Snetpoint/best.pkl'
                    torch.save(generator.state_dict(), gpath2)
                    print('Gnetbestsave')
                s_loss = bce_loss(output2,zeros)

            if epoch > S_epoch:
                g_loss = 0.5 * g_loss_d + 100 * g_loss_l1 + bceloss

                generator.zero_grad()
                g_loss.backward()
                g_optimizer.step()
            else:
                g_loss = g_loss_d + 100 * g_loss_l1
                generator.zero_grad()
                g_loss.backward()
                g_optimizer.step()
        ...
        if epoch > S_epoch:
            ...
            print('g_cond_loss:%f, g_loss_d:%f, bceloss:%f' % (g_loss_l1.item(), g_loss_d.item(), bceloss.item()))
            print('[%d/%d，%d/%d] - D_loss: %f,S_loss:%f, G_loss: %f' %
                  ((epoch + 1), train_epoch, i + 1, 15000 / batch_size, d_loss.item(), s_loss.item(), g_loss.item()))

        else:
            print('g_cond_loss:%f,g_loss_d:%f' % (g_loss_l1.item(), g_loss_d.item()))
        ...
        if i == 149:
            ...
            z1 = torch.randn((1, 1024, 8)).to(device)
            fake_speech = generator(testdata, z1)
            ...
            path = './chennetpoint/audio/' + str(epoch + 1) + '.wav'
            wavfile.write(path, 16384, fake_speech_data)
            print('audio' + str(epoch + 1) + 'save')
            ###保存权重
            gpath = './chennetpoint/Gnetpoint/net' + str(epoch + 1) + '.pkl'
            torch.save(generator.state_dict(), gpath)
            print('Gnet' + str(epoch + 1) + 'save')
            generator = generator.train()
            dpath = './chennetpoint/Dnetpoint/net' + str(epoch + 1) + '.pkl'
